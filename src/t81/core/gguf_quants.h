...
namespace t81 {
    namespace core {
        namespace gguf {
            ...
            inline std::uint16_t
            pack_block(const std::array<std::uint8_t, TQ1_TRITS_PER_BLOCK> &digits) {
            ...
            inline void unpack_block(std::array<std::uint8_t, TQ1_TRITS_PER_BLOCK> &digits,
                                     std::uint16_t value) {
            ...
            inline void quantize_row_tq1_0(const float *values, std::int64_t length,
                                           float threshold, float supplied_scale,
                                           std::uint16_t *destination) {
                if (length <= 0 || destination == nullptr) {
                    return;
                }
                const float scale =
                    supplied_scale > 0.0f ? supplied_scale : compute_scale(values, length);
                const float clamped_threshold = clamp_threshold(threshold);
                const float inv_scale = scale > 0.0f ? 1.0f / scale : 0.0f;
                const std::size_t blocks = static_cast<std::size_t>(
                    (length + TQ1_TRITS_PER_BLOCK - 1) / TQ1_TRITS_PER_BLOCK);

                for (std::size_t block_index = 0; block_index < blocks; ++block_index) {
                    std::array<std::uint8_t, TQ1_TRITS_PER_BLOCK> digits{};
                    for (std::size_t trit_index = 0; trit_index < TQ1_TRITS_PER_BLOCK;
                         ++trit_index) {
                        const std::size_t linear_index =
                            block_index * TQ1_TRITS_PER_BLOCK + trit_index;
                        std::int8_t trit = 0;
            ...
            inline void dequantize_row_tq1_0(const std::uint16_t *source, std::int64_t length,
