...
namespace {
    ...
    std::span<const float> make_const_float_span(py::buffer &buffer, std::size_t count) {
        const auto info = buffer.request(false);
        if (info.itemsize != static_cast<py::ssize_t>(sizeof(float))) {
            throw py::value_error("C buffer must store float32 values");
        }
        ...
        const auto *data =
            info.ptr ? reinterpret_cast<const float *>(info.ptr) : nullptr;
        return {data, count};
    }
    ...
    quantize_to_trits(py::array_t<float, py::array::c_style | py::array::forcecast> array,
                      float threshold) {
    ...
    py::array_t<float>
    dequantize_trits(py::array_t<std::int8_t, py::array::c_style | py::array::forcecast> array) {
        const auto info = array.request();
    ...
    pack_dense_matrix(py::array_t<float, py::array::c_style | py::array::forcecast> array,
                      float threshold) {
        if (threshold <= 0.0f || threshold > 1.0f) {
            throw py::value_error("threshold must be between 0 (exclusive) and 1");
        }
    ...
    py::array_t<float> dequant_tensor_tq1_impl(py::buffer buffer,
        ...
        const std::size_t group_rows =
            static_cast<std::size_t>(std::max<std::int64_t>(1, block_rows));
        std::size_t offset = 0;
        ...
        while (current_row < rows) {
            ...
            const std::size_t rows_in_group =
                static_cast<std::size_t>(std::min<std::int64_t>(group_rows, rows - current_row));
    ...
    template <typename Int> void bind_montgomery_context(py::module_ &module, const char *name) {
...
PYBIND11_MODULE(t81lib, module) {
    ...
    module.def(
        ...
        py::arg("x"),
        py::arg("y"),
        "Select between `x` and `y` per element based on `condition`.");
    ...
    module.def(
        ...
        py::arg("end"),
        py::arg("weight"),
        "Perform elementwise linear interpolation between start and end.");
    ...
    module.def(
        ...
        py::arg("tensor2"),
        py::arg("value") = 1.0f,
        "Compute input + value * tensor1 * tensor2 elementwise.");
    ...
    module.def(
        ...
        [](py::buffer buffer) {
            ...
            if (info.ndim != 1 || info.shape[0] != static_cast<py::ssize_t>(core::limb::BYTES)) {
                throw py::value_error("limb_from_bytes expects a byte array of length " +
    ...
    py_limb.def(py::init<>())
        ...
        .def_static("from_bytes",
                    [](py::buffer buffer) {
                        const auto info = buffer.request();
                        if (info.ndim != 1 ||
                            info.shape[0] != static_cast<py::ssize_t>(core::limb::BYTES)) {
    ...
    py_bigint.def(py::init<>())
        ...
        .def_static("from_trits",
                    [](py::array_t<std::int8_t, py::array::c_style | py::array::forcecast> trits) {
