...
bool check_limb_modular(const MontgomeryContext<limb>& ctx) {
    ...
    for (long long a_value : values) {
        ...
        for (long long b_value : values) {
            ...
            if (product != expected_limb) {
                ...
                std::cerr << "a=" << t81::io::to_string(a) << " b="
                          << t81::io::to_string(b) << "\n";
    ...
    const auto large_exponent = limb::from_value(
        static_cast<long long>(1) << guard.max_bits());
    bool threw = false;
    try {
    ...
    for (int iteration = 0; iteration < 8; ++iteration) {
        ...
        if (product != expected) {
            std::cerr << "bigint montgomery mul mismatch\n"
                      << "a=" << t81::io::to_string(a) << " b="
                      << t81::io::to_string(b) << "\n"
                      << "expected=" << t81::io::to_string(expected) << " got="
