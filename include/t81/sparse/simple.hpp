...
namespace t81::sparse {
    ...
    inline void spmm_simple(const SimpleSparseTernary &A,
        ...
        for (std::size_t entry = 0; entry < entry_count; ++entry) {
            ...
            const double weight = detail::limb_to_double(A.values[entry]) * alpha_d;
            ...
            for (std::size_t k = 0; k < n_size; ++k) {
                const double product = weight * static_cast<double>(B[col_offset + k]);
                const double accumulated = static_cast<double>(C[row_offset + k]) + product;
