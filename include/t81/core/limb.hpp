    ...
    class limb {
      public:
        ...
        limb consensus(const limb &other) const {
            return apply_tritwise(
                other, [](std::int8_t lhs, std::int8_t rhs) { return lhs == rhs ? lhs : 0; });
        }
      ...
      private:
        template <typename Fn> limb apply_tritwise(const limb &other, Fn &&fn) const {
...
namespace std {
    ...
    template <> class numeric_limits<t81::core::limb> {
      public:
    ...
    template <> struct hash<t81::core::limb> {
