...
namespace t81::core {
    ...
    template <typename Fn> bigint expected_bitwise(const bigint &, const bigint &, Fn);
    ...
    class bigint {
      public:
        ...
        explicit operator Int() const {
            ...
            const wide max_value = static_cast<wide>(std::numeric_limits<Int>::max());
            const wide min_value = static_cast<wide>(std::numeric_limits<Int>::min());
        ...
        bigint consensus(const bigint &other) const {
            return apply_limbwise(
                *this, other, [](const limb &lhs, const limb &rhs) { return lhs.consensus(rhs); });
        }
        ...
        bigint trit_shift_right(int count) const {
            ...
            if (limb_shift > 0) {
                ...
                if (shift_limbs >= result.limbs_.size()) {
                    ...
                    result.limbs_.erase(result.limbs_.begin(), result.limbs_.begin() + shift_limbs);
                    result.normalize();
                }
        ...
        template <typename Fn> friend bigint expected_bitwise(const bigint &, const bigint &, Fn);
      ...
      private:
        ...
        compare_magnitude_vectors(const std::vector<limb> &lhs,
        ...
        divide_magnitude(std::vector<limb> dividend, const std::vector<limb> &divisor) {
            ...
            while (true) {
                ...
                if (compare_magnitude_vectors(next_scaled, dividend) ==
                    std::strong_ordering::greater) {
                    break;
                }
            ...
            for (int index = static_cast<int>(scaled.size()) - 1; index >= 0; --index) {
                if (!remainder.empty() && compare_magnitude_vectors(remainder, scaled[index]) !=
    ...
    inline std::vector<std::uint8_t> bigint::to_bytes() const {
        ...
        // Store the limb count using the fixed-width header derived from numeric_limits so
        // protocols stay endianness-agnostic.
        ...
        // Append each balanced-ternary limb (3^48) in its canonical byte form so downstream
        // serializers can treat the blob as base-3^48 chunks.
        for (const auto &limb_value : limbs_) {
    ...
    inline bigint bigint::from_bytes(std::span<const std::uint8_t> data) {
        // Accept a span so callers can deserialize from std::vector, flatbuffers, or memory-mapped
        // blobs.
        constexpr std::size_t header_size = 1 + bigint::LIMB_COUNT_BYTES;
...
namespace std {
    ...
    template <> struct hash<t81::core::bigint> {
    ...
    template <> struct formatter<t81::core::bigint> {
        bool ternary = false;
