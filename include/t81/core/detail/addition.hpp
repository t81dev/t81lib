...
namespace t81::core::detail {
    ...
    inline constexpr std::uint32_t encode_outcome(const AddOutcome &outcome) noexcept {
        ...
        const std::uint32_t carry =
            static_cast<std::uint32_t>(outcome.carry + 1); // map -1,0,1 -> 0,1,2
    ...
    inline constexpr std::uint32_t select_packed_outcome(std::uint32_t packed_entry,
                                                         int carry_in) noexcept {
    ...
    inline constexpr std::uint32_t compose_packed_maps(std::uint32_t left,
