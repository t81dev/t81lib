...
namespace t81::core::detail {
    ...
    inline bool gather_packed_additions_avx512(const limb &lhs, const limb &rhs,
        ...
        for (int offset = 0; offset < limb::TRYTES; offset += 16) {
            ...
            const __m512i gather = _mm512_i32gather_epi32(index_vec, base_ptr, 4);
    ...
    inline std::pair<limb, limb>
    pair_from_accumulator(const std::array<int, limb::TRITS * 2> &accum) {
...
#if defined(__AVX512F__)
    inline std::optional<std::pair<limb, limb>> mul_wide_avx512(const limb &lhs, const limb &rhs) {
        std::array<int, limb::TRITS * 2> accum{};
...
#else
    inline std::optional<std::pair<limb, limb>> mul_wide_avx512(const limb &, const limb &) {
        return std::nullopt;
    }
#endif
