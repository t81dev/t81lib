...
namespace t81::core::montgomery {
    ...
    template <typename Int> class MontgomeryConstTimeGuard;

    template <> class MontgomeryConstTimeGuard<limb> {
      public:
        ...
        void require(const limb &exponent) const {
            if (!allows(exponent)) {
                throw std::domain_error(
                    "MontgomeryConstTimeGuard<limb>: exponent exceeds guarded bit width");
            }
        }
    ...
    template <> class MontgomeryConstTimeGuard<bigint> {
      public:
        ...
        void require(const bigint &exponent) const {
            if (!allows(exponent)) {
                throw std::domain_error(
                    "MontgomeryConstTimeGuard<bigint>: exponent exceeds guarded bit width");
            }
        }
